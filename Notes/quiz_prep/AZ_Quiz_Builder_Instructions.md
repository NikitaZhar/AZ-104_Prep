# Azure Cert — Инструкция по сборке и проверке Quiz и Practice Assessment (самодостаточная)

Этот файл — **единственный и полный файл-задание для исполнителя (Claude)**, рассчитанный
на два равноправных сценария: **создание** нового Quiz/Practice Assessment с нуля и
...
Назначение: для каждого модуля курса (AZ-104 и далее) готовятся **два отдельных
HTML-файла** — **Quiz** (понимание материала) и **Practice Assessment** (практические
навыки). Оба используют один и тот же движок из раздела 7. Отличаются содержанием
и набором типов вопросов.
...
2. **Готовлю вопросы** как JSON-массив (формат — раздел 4), отдельно для Quiz и отдельно
   для Practice Assessment, по правилам разделов 1–3.
...
поддерживаемые/неподдерживаемые сценарии. **Не** Practice Assessment.
...
- **Типы вопросов** — Best answer (single choice) и Choose Two/Three (несколько верных).
  ...
  Cmdlet/Parameter Selection — это режимы Practice Assessment.
...
## 2. Practice Assessment — назначение и правила

Проверяет **практические навыки**: команды/cmdlet'ы (если реально есть в материале),
выбор инструмента под сценарий, последовательность действий в портале/CLI, решение в
сценарии («что сделать», «чего не делать»).
...
- **Если для модуля нет материала под какой-то режим — режим не используется.** Это не
  пробел, а корректное решение. Особенно про Parameter Selection: нет реальных
  cmdlet-параметров в источнике → режим пропускается целиком. Ничего не выдумывать.
...
Поле `section` для Practice Assessment содержит **название режима**
(`Single Choice` / `Choose Two` / `Tool Selection` / `Complete Solution` / ...),
а не тему подраздела.

---

## 3. Общие правила качества — для ВСЕХ вопросов (Quiz и Practice Assessment)
...
в разных формулировках. Если факт уже проверен в Quiz (понимание), в Practice Assessment
...
Assessment — это дубль. Quiz проверяет множественность как понимание, Practice Assessment —
...
```json
{
  "section": "Quiz: тема подраздела | Practice Assessment: название режима",
...
- ☐ Полнота покрытия подразделов источника (выровнено, пропусков нет)
- ☐ Нет вопросов вне материала модуля
- ☐ Нет повторяющихся вопросов/идей (в т.ч. между Quiz и Practice Assessment)
- ☐ Choose Two/Three только там, где источник перечисляет несколько верных фактов
- ☐ Practice Assessment: использованы только режимы, подтверждённые материалом
...
- Quiz: `NNN_AZ104_<Path>_<ModuleName>.html`
- Practice Assessment: `NNN_AZ104_<Path>_<ModuleName>_PracticeAssessment.html`

Номер `NNN` — одинаковый для обоих файлов одного модуля.
